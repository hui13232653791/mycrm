...
import com.wangyh.crm.dao.CustomerMapper;
import com.wangyh.crm.dao.CustomerServeMapper;
import com.wangyh.crm.dao.UserMapper;
import com.wangyh.crm.enums.CustomerServeStatus;
import com.wangyh.crm.query.CustomerServeQuery;
import com.wangyh.crm.utils.AssertUtil;
import com.wangyh.crm.vo.CustomerServe;
...
public class CustomerServeService extends BaseService<CustomerServe,Integer> {
    ...
    public void updateCustomerServe(CustomerServe customerServe) {
        ...
        // 判断客户服务的服务状态
        if (CustomerServeStatus.ASSIGNED.getState().equals(customerServe.getState())) { //服务分配状态 fw_002
            // 服务分配操作
            // 分配人       非空，分配用户记录存在
        ...
        } else if (CustomerServeStatus.PROCED.getState().equals(customerServe.getState())) { //服务处理状态 fw_003
            // 服务处理操作
            // 服务处理内容   非空
            AssertUtil.isTrue(StringUtils.isBlank(customerServe.getServiceProce()), "服务处理内容不能为空！");
            // 服务处理时间   系统当前时间
            customerServe.setServiceProceTime(new Date());
        } else if (CustomerServeStatus.FEED_BACK.getState().equals(customerServe.getState())) { //服务反馈状态  fw_004
            // 服务反馈操作
            // 服务反馈内容   非空
